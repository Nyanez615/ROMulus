...
- **Unreleased** — Onboarding Continue-button fix, `pre-push` cargo-sweep hook, Windows title bar fix (native menu macOS-only, window title populated), sidebar icon-only brand mark (doubles as collapse toggle, aligned identically in both states), onboarding Back buttons on all steps; **Archive extraction/compression** — new Extract/Compress bulk actions in the ROMs tab (`commands/archive.rs`, `ArchiveActionDialog.tsx`): flat extraction/compression (no per-archive subfolder), own FilterBar + Starts-With-letter batching independent of the page's filters, automatic already-extracted/already-compressed detection with a dedicated redundant-file cleanup flow, live per-file disk-space warnings netted against the delete-after toggle, per-row done/failed markers, byte-weighted countdown ETA, Windows read-only-attribute-aware deletion retry; Prune now prefers the extracted/native file over its zip twin (`score_rom`'s `extracted_bonus`, flipped from the prior zip-preferring `compressed_bonus`); **App-wide filter/sort/UX consistency pass** — `FilterBar`/`SortControl` unified across ROMs, System Files, History, Journal, and Downloads (replacing each tab's hand-rolled chip row); `AlphabetScrubber` retired in favor of the same Starts-With `FilterBar` chip everywhere (ROMs, System Files, Journal); Downloads' qBittorrent preview gained the same region/language/category/Starts-With chips on all four sub-tabs (Titles/All/Download/Skip), backed by extending `QbtGroupInfo`/`QbtFileDecision` with the underlying `RomFile` metadata; shared `pluralize()` helper (`src/lib/pluralize.ts`); ROMs' title bar brought into compliance with the canonical tab-layout pattern; **Scoring/grouping fixes** (all found by auditing the user's real 3DS/DS collection through the new Downloads filters) — pre-release/bad-dump/unofficial tiers now fall back to region-inferred language in their tiebreak (matching `matches_preferred`'s own fallback, via `preferred_lang_count()`); bare 4-character cartridge serial codes (`is_bare_serial_code()`) and "NDSi Enhanced"/"NDSi Exclusive" tags no longer trip the generic distribution-variant penalty; `build_date` now packs `YYYYMMDD * 1_000_000 + HHMMSS` (was `u32` YYYYMMDD, discarding time-of-day) so same-day builds tiebreak correctly; the generic unrecognized-extra_tag penalty no longer suppresses `revision_bonus` (only the confident `COLLECTION_TAGS`/`FORMAT_VARIANT_TAGS` tiers still do); Proto now beats Demo and undated now beats dated in otherwise-tied pre-release/unofficial ties; title-grouping normalization added for spelled-out sequel numbers ("Two".."Twenty") and "Math"/"Maths" (see Key Conventions below for what this normalization deliberately does *not* cover)
...
From `src-tauri/`:
```bash
cargo test                    # 316 unit tests + regenerates src/lib/bindings/
...
src/                             React frontend (Vite root)
  components/
    ...
    FilterBar.tsx                Collapsible, dimension-agnostic filter panel (groups: FilterGroup[] + leading/trailing slots); used by ROMs, System Files, History, Journal, Downloads, and ArchiveActionDialog
  ...
  lib/
    ...
    romFilters.ts                matchesLang/Region/Status/StartingLetter — structurally-typed predicates (Pick<RomGroup, ...>, not RomGroup itself) so any shape with the right fields can reuse them; matchesPreferred stays RomGroup-specific (needs has_preferred_version). Consumers: Roms.tsx, ArchiveActionDialog, Downloads.tsx (via a local adapter over QbtGroupInfo/QbtFileDecision, which already carry the same field names). STARTING_LETTERS/startingLetter() are the app-wide # + A–Z bucketing, also used directly against flat RomFile/PlayEntry lists (SystemFiles.tsx, Journal.tsx)
...
- **Title-grouping normalization is same-language only** — `picker.rs::normalize_key`'s word-equivalence handling (British/American spelling like colour→color, "math"↔"maths", spelled-out sequel numbers like "Two"→2) exists to merge genuinely-the-same text spelled differently *within* one language. It deliberately does **not** extend to cross-language translations of a title (e.g. "Tamagotchi Connexion" (French, Europe) vs "Tamagotchi Connection" (USA) — same game, different official localized titles) — that was tried and explicitly reverted. The two look similar but are a different kind of inference: same-word/same-language normalization is a safe, mechanical text transform; cross-language title-aliasing would require a real title-identity mapping (or metadata-database matching), not string normalization, and risks silently merging titles that only coincidentally look related.
...
## Testing
- Rust: `cargo test` in `src-tauri/` — 316 tests, in-memory SQLite only
