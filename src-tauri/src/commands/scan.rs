...
/// TEMPORARY DEBUG — find game groups counted by get_roms() for a canonical
/// prefix but missing from the group.console-based canonical_game attribution.
/// Returns (title_normalized, group_console) pairs that show up in variant
/// attribution but not in the primary-console attribution for `prefix`.
/// Remove once the GBA off-by-1 is diagnosed.
#[tauri::command]
pub fn debug_game_groups(state: State<'_, AppState>, prefix: String) -> Vec<(String, String)> {
    use crate::models::FileCategory;
    use std::collections::{HashMap, HashSet};

    let cache = state.scan_cache.lock().unwrap();

    // Attribution via group.console only (current approach)
    let mut console_attr: HashMap<String, HashSet<String>> = HashMap::new();
    for group in &cache.groups {
        if group
            .variants
            .iter()
            .any(|v| matches!(v.file_category, FileCategory::Game))
        {
            ...
            console_attr
                .entry(base)
                .or_default()
                .insert(group.title_normalized.clone());
        }
    }

    // Attribution via ALL variant consoles
    let mut variant_attr: HashMap<String, HashSet<String>> = HashMap::new();
    for group in &cache.groups {
        if group
            .variants
            .iter()
            .any(|v| matches!(v.file_category, FileCategory::Game))
        {
            let consoles: HashSet<String> = group
                .variants
                .iter()
                .map(|v| strip_format_suffix(&v.console).to_owned())
                .collect();
            for base in consoles {
                variant_attr
                    .entry(base)
                    .or_default()
                    .insert(group.title_normalized.clone());
    ...
    let base_key = prefix.as_str();
    let in_console = console_attr.get(base_key).cloned().unwrap_or_default();
    let in_variant = variant_attr.get(base_key).cloned().unwrap_or_default();

    // Titles found via variant attribution but not via console attribution
    let mut missing: Vec<(String, String)> = vec![];
    for title in &in_variant {
        if !in_console.contains(title) {
            // Find the group's actual console to explain the miss
            if let Some(group) = cache.groups.iter().find(|g| &g.title_normalized == title) {
                missing.push((title.clone(), group.console.clone()));
            }
        }
    }
    missing.sort();
